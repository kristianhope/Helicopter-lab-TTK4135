 ...
 * C source code generated on : Wed Feb  9 08:52:03 2022
 *
...
void helicopter_output(void)
{
  ...
  if (rtmIsMajorTimeStep(helicopter_M)) {
    ...
    /* S-Function Block: helicopter/HIL Read Analog (hil_read_analog_block) */
    {
      t_error result = hil_read_analog(helicopter_DW.HILInitialize_Card,
        &helicopter_P.HILReadAnalog_channels, 1,
        &helicopter_DW.HILReadAnalog_Buffer);
      if (result < 0) {
        msg_get_error_messageA(NULL, result, _rt_error_message, sizeof
          (_rt_error_message));
        rtmSetErrorStatus(helicopter_M, _rt_error_message);
      ...
      helicopter_B.HILReadAnalog = helicopter_DW.HILReadAnalog_Buffer;
    }

    /* S-Function (hil_read_encoder_block): '<Root>/HIL Read Encoder' */

    /* S-Function Block: helicopter/HIL Read Encoder (hil_read_encoder_block) */
    {
      t_error result = hil_read_encoder(helicopter_DW.HILInitialize_Card,
        &helicopter_P.HILReadEncoder_channels, 1,
        &helicopter_DW.HILReadEncoder_Buffer);
      if (result < 0) {
        msg_get_error_messageA(NULL, result, _rt_error_message, sizeof
          (_rt_error_message));
        rtmSetErrorStatus(helicopter_M, _rt_error_message);
      } else {
        helicopter_B.HILReadEncoder = helicopter_DW.HILReadEncoder_Buffer;
      }
    }
  }
}
...
RT_MODEL_helicopter_T *helicopter(void)
{
  ...
  /* External mode info */
  helicopter_M->Sizes.checksums[0] = (2226957924U);
  helicopter_M->Sizes.checksums[1] = (2188352731U);
  helicopter_M->Sizes.checksums[2] = (3728478050U);
  helicopter_M->Sizes.checksums[3] = (1729431675U);
...

  {
    ...
    helicopter_B.HILReadAnalog = 0.0;
    helicopter_B.HILReadEncoder = 0.0;
    helicopter_B.In1 = 0.0;
  }
  ...
  helicopter_DW.HILReadAnalog_Buffer = 0.0;
